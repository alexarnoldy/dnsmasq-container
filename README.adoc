...
CAUTION: Deploying a DHCP service on a subnet that already has one can cause mind-numbingly-difficult-to-troubleshoot hassles and make the offender surprisingly unpopular. See the Troubleshooting section below to verify that no other DHCP services are running on your subnet.
